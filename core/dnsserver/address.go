...
type zoneAddr struct {
	...
	HostBits  int    // if reverse zone, this hold the mask when not a multiple of 8
}
...
func normalizeZone(str string) (zoneAddr, error) {
	...
	host, port, bits, err := plugin.SplitHostPort(str)
	if err != nil {
		return zoneAddr{}, err
	}
	...
	return zoneAddr{Zone: dns.Fqdn(host), Port: port, Transport: trans, HostBits: bits}, nil
}
