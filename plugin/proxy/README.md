...
* `coredns_proxy_request_duration_seconds{server, proto, proto_proxy, family, to}` - duration per upstream
  interaction.
...
and family the transport family ("1" for IPv4, and "2" for IPv6). `Server` is the server handling
the request, see the documentation of the *metrics* plugin.
