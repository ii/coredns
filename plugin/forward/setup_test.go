...
func TestSetupTLS(t *testing.T) {
	...
	for i, test := range tests {
		...
		if !test.shouldErr && test.expectedServerName != "" && test.expectedServerName != f.proxies[0].health.(*hc).c.TLSConfig.ServerName {
			t.Errorf("Test %d: expected: %q, actual: %q", i, test.expectedServerName, f.proxies[0].health.(*hc).c.TLSConfig.ServerName)
		}
	}
}
